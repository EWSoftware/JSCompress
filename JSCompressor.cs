	...
	public class JSCompressor
	{
        ...
        public string Compress(string strScript)
        {
            ...
            if(reInsLit == null)
            {
                ...
                reFuncParams = new Regex(@"function(?:\s+\w*?|\s*?)\((.*?)\)(.*?|\n)?\{", RegexOptions.Singleline);
                reFindVars = new Regex(@"(var\s+.*?)(;|\sin\s|$)", RegexOptions.Multiline | RegexOptions.IgnoreCase);
                reStripVarPrefix = new Regex(@"^var\s+");

                // This uses balancing groups to properly match nested parentheses and brackets
        ...
        private string CompressVariables(string script)
        {
            ...
            foreach(string replaceName in scVariables)
            {
                ...
                // Don't bother if the existing name is shorter.  This check could be removed
                // to obfuscate the variable name even if it would be longer.
                if(name.Length < replaceName.Length)
                {
                    incVarName = true;
                    script = Regex.Replace(script, @"([^\w\.])" + replaceName + @"(?=\W)", "$1" + name);
                }
            }
